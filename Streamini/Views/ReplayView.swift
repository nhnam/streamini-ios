...
class ReplayView: UIView {
    ...
    override func awakeFromNib() {
        ...
        playButton.addTarget(self, action: #selector(ReplayView.playButtonPressed(_:)), for: UIControlEvents.touchUpInside)
        closeButton.addTarget(self, action: #selector(ReplayView.closeButtonPressed(_:)), for: UIControlEvents.touchUpInside)
        viewersButton.addTarget(self, action: #selector(ReplayView.viewersButtonPressed(_:)), for: UIControlEvents.touchUpInside)
        replaysButton.addTarget(self, action: #selector(ReplayView.replaysButtonPressed(_:)), for: UIControlEvents.touchUpInside)
    ...
    func playButtonPressed(_ sender: UIView) {
        if let del = delegate {
    ...
    func closeButtonPressed(_ sender: UIView) {
        if let del = delegate {
            del.replayViewCloseButtonPressed(self)
        }
    }
    
    func viewersButtonPressed(_ sender: UIView) {
        if let del = delegate {
    ...
    func replaysButtonPressed(_ sender: UIView) {
        if let del = delegate {
