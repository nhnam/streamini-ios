...
class LiveStreamViewController: BaseViewController, UserSelecting, UserStatusDelegate, UIAlertViewDelegate {
    ...
    func forceClose(_ notification: Notification) {
    ...
    func pingFailure(_ error: NSError) {
        ...
        if let userInfo = error.userInfo as? [NSObject: NSObject] {
            ...
            if userInfo["code" as NSObject] as? UInt == Error.kUnsuccessfullPing {
    ...
    func configureView() {        
        closeButton.setImageTintColor(UIColor(white: 1.0, alpha: 1.0), for: UIControlState())
        closeButton.setImageTintColor(UIColor(white: 1.0, alpha: 0.5), for: UIControlState.highlighted)
        rotateButton.setImageTintColor(UIColor(white: 1.0, alpha: 1.0), for: UIControlState())
        rotateButton.setImageTintColor(UIColor(white: 1.0, alpha: 0.5), for: UIControlState.highlighted)
        infoButton.setImageTintColor(UIColor(white: 1.0, alpha: 0.7), for: UIControlState())
        infoButton.setImageTintColor(UIColor(white: 1.0, alpha: 1.0), for: UIControlState.highlighted)
        eyeButton.setImageTintColor(UIColor(white: 1.0, alpha: 0.7), for: UIControlState())
    ...
    fileprivate func removeCommentAt(_ indexPath: IndexPath) {
        ...
        commentsTableView.deleteRows(at: [indexPath], with: UITableViewRowAnimation.fade)
    }
