...
UIActionSheetDelegate, SelectFollowersDelegate, ReplayViewDelegate, UserSelecting, CollectionViewPullDelegate, PayPalPaymentDelegate {
    ...
    func joinFailure(_ error: NSError) {
        ...
        if let userInfo = error.userInfo as? [NSObject: NSObject] {
            ...
            if userInfo["code" as NSObject] as? UInt == Error.kUserBlocked {
                UIAlertView.userBlockedAlert().show()
                self.dismiss(animated: true, completion: nil)
            }
            if userInfo["code" as NSObject] as? UInt == Error.kPaidStream {
                buyAccess()
            }
        } else {
    ...
    func forceLeave(_ notification: Notification) {
        if streamPlayer != nil {
